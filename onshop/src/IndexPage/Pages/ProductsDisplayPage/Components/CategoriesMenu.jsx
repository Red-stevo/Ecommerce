...
const CategoriesMenu = () => {
    ...
    const [visibleCategories, setVisibleCategories] = useState(productCategory);
    const [overflowingCategories, setOverflowingCategories] = useState([]);
    const [screeResized, setScreenResized] = useState(false);


    useEffect(() => {
        const detectOverflow = () => {
            if (categoryMenuRef.current || screeResized) {
                ...
                productCategory.forEach((item, index) => {
                    const child = children[index + 1];
                    if (child) {
                        const childRect = child.getBoundingClientRect();
                        if (childRect.right > containerRect.right || childRect.bottom > containerRect.bottom)
                            newOverflowing.push(item);
                        else newVisible.push(item);
                    }
                });
    ...
    }, [productCategory, screeResized]);



    useEffect(() => {
        const handleResize = () => {
            setVisibleCategories(productCategory);
    ...
    return (
        <div className={"product-category-header"}>
            <div className="product-category-menu" ref={categoryMenuRef}>
                <div className="categories">
                    <div className="category-name">Home</div>
                    <FcHome height={20} width={20} className="category-icon" />
                </div>
                {visibleCategories.map(({ category, categoryId, iconUrl }, index) => (
                    <div key={index} className="categories">
                        <div className="category-name">{category}</div>
                        <Image src={iconUrl} alt={`${category} icon`} height={20} width={20} className="category-icon" />
                    </div>
                ))}
            </div>
            <RxDoubleArrowRight className={"more-categories"} />
        </div>
    );
};
